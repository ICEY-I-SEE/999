        ...
        <div v-show="logistics!=''" class="content">
            <div class="box pt">
                <div class="order-info" v-for="(item,key) in logistics.goods_res" :key="key">
                    ...
                    <div class="right-detail">
                        <p><b>物流状态：</b>
                            <span v-if="wuliu.order_type==1" class="txt-success">商品已出库</span>
                            <span v-if="wuliu.order_type==2" class="txt-success">商品发货中</span>
                        </p>
                        <p>快递公司：{{wuliu.com}}</p>
                        <p>快递单号：{{wuliu.invoice_no}}</p>
                    </div>
            ...
            <div class="box mt">
                <div class="logistics-info">
                    <ul class="-info-" v-if="wuliu.data">
                        <li class="active" v-for="(item,index) in wuliu.data" :key="index">
                            <i class="-icon" :class="{'-icon-active':index==0}"></i>
                            <p class="txt"><span>{{item.status}}</span> {{item.context}}</p>
                            <!-- <p class="txt"></p> -->
                            <p class="time">{{item.time}}</p>
                        </li>
                    </ul>
                    <div v-else>{{wuliu.message}}</div>
                </div>
            </div>
